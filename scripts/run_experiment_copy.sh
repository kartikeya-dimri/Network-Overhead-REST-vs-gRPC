...
run_space() {
  ...
  # Ensure output directories exist before any redirection
  ...
  for proto in "${PROTOCOLS[@]}"; do
    ...
    for size in "${PAYLOAD_SIZES[@]}"; do
      ...
      # || true prevents SIGPIPE (from head -20 closing early) from being treated
      # as a script failure under set -o pipefail
      ${K6_BIN} run \
        -e "PROTOCOL=${proto}" \
...
run_time() {
  ...
  # Ensure output directory exists before any redirection
  ...
  for proto in "${PROTOCOLS[@]}"; do
    for struct in "${STRUCTURES[@]}"; do
      ...
      for size in "${PAYLOAD_SIZES[@]}"; do
        ...
        # || true prevents a non-zero pipe exit status (e.g. k6 crash mid-run, or
        # sed finding no matches) from killing the script under set -o pipefail
        ${K6_BIN} run \
          -e "PROTOCOL=${proto}" \
          -e "SERVER_IP=${SERVER_IP}" \
          -e "PAYLOAD_SIZE=${size}" \
